      ...
      <van-swipe-cell>
        <div class="goods-card">
          <div class="venter_top">
            <div class="enter_id">出库单号: <span class="enter_name">{{ item.logistics_no }}</span></div>
            <!-- <div class="enter_name">测试仓库</div> -->
          </div>
          <div class="enter_info">
            <div class="enter_img">
              <van-image width="100%" height="100%" :src="require('@/assets/images/stock-out.png')"/>
            </div>
            <div class="enter_user">
              <!-- <div class="enter_num">X 30</div> -->
              <div class="enter_limitation enter_num_wid">收货人：<span class="enter_name">{{ item.contacts }}</span></div>
        ...
        <template #right v-if="item.status == 1">
          <van-button square color="#909399" size="normal" text="撤销" class="delete-button" @click="delt(item.wo_id)"/>
        </template>
      </van-swipe-cell>
...
import { getOutList, cancel } from '@/api/inventory'
...
export default {
  ...
  methods: {
    ...
    obtain () {
      ...
      getOutList(that.inven).then(res => {
        if (res.state == 300) {
          setTimeout(function () {
            that.obtain()
          }, 1000)
    ...
    // 创建出库单
    addenter () {
      this.$router.push({ path: '/gout_add' })
    },

    delt (e) {
      const that = this
      that.$dialog.confirm({ title: '温馨提示', message: '此操作将撤回出库信息，是否继续操作' })
        .then(() => {
          that.loading_status = true
          cancel({ wo_id: e }).then(res => {
            that.loading_status = false
            that.$toast(res.msg)
            if (res.state == 200) {
              that.obtain()
